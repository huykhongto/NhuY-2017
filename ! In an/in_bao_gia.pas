...
{the following procedures are needed for communication with bao_gia3D;
...
begin
  ...
  for i := 0 to maxgraf do with bao_giadata[i] do
    begin
 ...
 begin
	...
	begin
		...
		begin
			...
			tong:=chuoi_so_n(sl_tk)*chuoi_so_n(dg_tk);
...
begin
  ...
  bao_giadata[igraph].ok := grafok;
  bao_giadata[igraph].x1 := round((x1-xoff)/res);
  bao_giadata[igraph].x2 := round((x2-xoff)/res);
  bao_giadata[igraph].y1 := round((y1-yoff)/res);
  bao_giadata[igraph].y2 := round((y2-yoff)/res);
  bao_giadata[0] := bao_giadata[igraph];
  {xyunzoom(igraph);}
  stil := st;
end;
...
begin {------------------------------------------------------------------------}
   ...
   with bao_giadata[igraph].xaxis do
...
begin {------------------------------------------------------------------------}
   ...
   with bao_giadata[igraph].yaxis[code] do
...
begin
  ...
  with bao_giadata[igraph].yaxis[1] do
...
begin
  ...
  with bao_giadata[igraph].xaxis do with graphs[igraph] do
...
procedure initbao_gia; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
...
begin                    { <--- }
  initbao_gia;           { <--- }
  {for C++ users: discard these two lines and call initbao_gia from your
