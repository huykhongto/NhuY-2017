...
uses Windows , {Messages,} SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
		{StdCtrls, Spin,} ExtCtrls, Clipbrd{, AppEvnts}, xycommon, in_hoa_don,System.Types,
		System.UITypes;
...
begin {------------------------------------------------------------------------}
  ...
  if dozoom then {wis oud beeld} with in_hoa_dondata[igraph] do
    begin
      copyzoom(igraph,0,1,0,1);
...
begin
  ...
  for i := 0 to np-1 do
    begin
			x1 := r*cos(i/np*2*pi+d); y1 := r*sin(i/np*2*pi+d);
...
var x1,x2,y1,y2,z2 : single;
begin
	x1 := r*cos(i/np*2*pi+dh); y1 := r*sin(i/np*2*pi+dh);
  x2 := x1; y2 := y1*coh; z2 := y1*sih; {kantelen}
