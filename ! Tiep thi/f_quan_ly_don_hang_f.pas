...
type
  Tf_quan_ly_don_hang = class(TForm)
    lv_don_hang: TListView;
    Panel1: TPanel;
    Splitter1: TSplitter;
    cbb_tim: TComboBox;
    b_tim: TButton;
    ...
    Panel4: TPanel;
    Splitter2: TSplitter;
    lv_don_hang_tim: TListView;
    cb_tim_nang_cao: TCheckBox;
    Panel5: TPanel;
    Label7: TLabel;
    l_von: TLabel;
    cbb_nhom_don_hang: TComboBox;
    ...
    procedure lv_don_hangMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure b_suaClick(Sender: TObject);
    procedure lv_don_hangColumnClick(Sender: TObject; Column: TListColumn);
    procedure lv_don_hangCompare(Sender: TObject; Item1, Item2: TListItem;
    ...
    procedure lv_don_hang_timMouseDown(Sender: TObject; Button: TMouseButton;
			Shift: TShiftState; X, Y: Integer);
		function tinh_tong_von :boolean;
    procedure l_vonClick(Sender: TObject);
    procedure cbb_nhom_don_hangChange(Sender: TObject);
	private
...
var
	f_quan_ly_don_hang: Tf_quan_ly_don_hang;
	ma_don_hang:string;
	ma_don_hang_ht:string; // ma thấu kính hiển thị
	da_tim:boolean;
	vi_tri_xem:integer;
	tu_vua_tim:string;
...
function Tf_quan_ly_don_hang.tinh_tong_von :boolean;
	var
	i:integer;
	tong:integer;
	don_gia:integer;
...
begin
	tong:=0;
	for I := 0 to lv_don_hang.Items.Count-1 do
	begin
		ListItem := lv_don_hang.Items[i];
...
//======================================== 
procedure Tf_quan_ly_don_hang.doc_thong_so;
var 
	dong:string;
...
begin
	...
	f_quan_ly_don_hang.Left:=tach_s(a[1]);
	f_quan_ly_don_hang.top:=tach_s(a[2]);
	ma_don_hang:= tach_c_md(a[3],'VT000000');
	for i := 0 to 9 do
	begin
		lv_don_hang.Columns[i].Width:=tach_s_md(a[i+4],50);
	end;
end;
//======================================== 
procedure Tf_quan_ly_don_hang.luu_thong_so;
var
	dong:string;
	i:integer;
...
begin
	if cbb_nhom_don_hang.ItemIndex=1 then
	//f_dl.lb_don_hang_tai;
	dong:=phien_ban_du_lieu+phan_cot;
	them_s(f_quan_ly_don_hang.Left); // 1
	them_s(f_quan_ly_don_hang.Top); // 2
	them_c(ma_don_hang); // 3
	for i := 0 to 9 do
	begin
		them_s(lv_don_hang.Columns[i].Width);
	end;
	f_dl.lb_don_hang.Items[0]:=dong; 
	f_dl.lb_don_hang_luu;
	if cbb_nhom_don_hang.ItemIndex=1 then
	begin
		//f_dl.lb_don_hang_tai1;
	end;
	//them_c('1111');
end;
//======================================== 
procedure Tf_quan_ly_don_hang.them_dl(lua_chon: byte);
var
	ngay_cn:string;
	dong_dl:string;
...
begin
	...
	begin
		begin
			...
			if lua_chon=1 then
			begin
				ma_don_hang:=tang_chuoi_so_5(ma_don_hang);
				f_dl.lb_don_hang.Items.Add(dong_dl);
			end
			else
			begin
				lb_sua(f_dl.lb_don_hang,dong_dl);
			end;
		end;
		//f_dl.lb_thau_kinh_luu;
		tm1.Enabled := true;
		sttb1.Panels[0].Text := 'Thành công';
		lb_so_kh.Caption := 'Số Vật Tư: ' +IntToStr(f_dl.lb_don_hang.Items.Count-1);
		lb_lv(f_dl.lb_don_hang,lv_don_hang);
		if cbb_nhom_don_hang.ItemIndex=1 then
			//f_dl.lb_don_hang_luu1;
		luu_thong_so;
	end;
end;
//======================================== 
procedure Tf_quan_ly_don_hang.lv_don_hangMouseDown(Sender: TObject;
...
var
	...
	khac:string;
begin
	if lv_don_hang.itemindex <> -1 then
	begin
		ListItem := lv_don_hang.Items[lv_don_hang.ItemIndex];
		with ListItem do
		begin
			ma_don_hang_ht:=Caption;
...
procedure Tf_quan_ly_don_hang.lv_don_hang_timMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
	ListItem: TListItem;
	thong_tin: string;
	khac:string;
begin
	if lv_don_hang_tim.itemindex <> -1 then
	begin
		ListItem := lv_don_hang_tim.Items[lv_don_hang_tim.ItemIndex];
		with ListItem do
		begin
			ma_don_hang_ht:=Caption;
			cbb_ten.Text:= SubItems.Strings[0];
...
procedure Tf_quan_ly_don_hang.l_vonClick(Sender: TObject);
begin
	tinh_tong_von;
end;

//---------------------------------------- 
procedure Tf_quan_ly_don_hang.tm1Timer(Sender: TObject);
begin
	sttb1.Panels[0].Text := '';
	tm1.Enabled := false;
end;
...
//======================================== 
//---------------------------------------- 
procedure Tf_quan_ly_don_hang.b_timClick(Sender: TObject);
var 
	i:integer;
	vi_tri:integer;
	thong_tin: string;
...
begin
	...
	begin
		...
		begin
			...
			so_tu_tim_thay:=lv_don_hang_tim.Items.Count;
...
//				nghia_tu:=LowerCase(nghia_tu);
			...
			lv_don_hang_tim.CustomSort(@Sap_xep_so_lv,-10);
			so_tu_tim_thay:=lv_don_hang_tim.Items.Count;
			//l_tim_thay.Caption:='Tìm thấy:'+IntToStr(so_tu_tim_thay)+' mục';
    end;
	end;
...
//======================================== 
procedure Tf_quan_ly_don_hang.b_nhapClick(Sender: TObject);
	var
	ngay_cn:string;
	dong_dl:string;
	dong_thong_tin:string;
...
procedure Tf_quan_ly_don_hang.b_suaClick(Sender: TObject);
begin
...
procedure Tf_quan_ly_don_hang.b_themClick(Sender: TObject);
begin
	them_dl(1);
	tinh_tong_von;
end;

procedure Tf_quan_ly_don_hang.b_xoa_mucClick(Sender: TObject);
begin
	if lv_don_hang.ItemIndex>=0 then
	begin
		if MessageDlg('Bạn có muốn xóa mục này!?', mtConfirmation, [mbYes, mbNo], 0) = idYes
		then
		begin
			lb_xoa(f_dl.lb_don_hang,ma_don_hang_ht);
			if cbb_nhom_don_hang.ItemIndex=0 then
			begin
				f_dl.lb_don_hang_luu;
			end;
			if cbb_nhom_don_hang.ItemIndex=1 then
			...
			luu_thong_so;
			lb_so_kh.Caption := 'Số Thấu kính: ' + IntToStr(f_dl.lb_don_hang.Items.Count-1);
			lv_don_hang.Items.Delete(lv_don_hang.itemindex);
		end;
	end;
	tinh_tong_von;
end;

procedure Tf_quan_ly_don_hang.cbb_gia_banChange(Sender: TObject);
begin
	cbb_gia_ban.Hint:=so_ra_chu(strtoint64(cbb_gia_ban.Text),sai,dung);
end;

procedure Tf_quan_ly_don_hang.cbb_gia_vonChange(Sender: TObject);
begin
	cbb_gia_von.Hint:=so_ra_chu(strtoint64(cbb_gia_von.Text),sai,dung);
end;

procedure Tf_quan_ly_don_hang.cbb_nhom_don_hangChange(Sender: TObject);
begin
	if cbb_nhom_don_hang.ItemIndex=0 then
	begin
		f_dl.lb_don_hang.Clear;
		//f_dl.lb_don_hang_tai;
	end;
	if cbb_nhom_don_hang.ItemIndex=1 then
	begin
	...
	//f_dl.lb_don_hang_luu_lv(lv_don_hang);
	lb_lv(f_dl.lb_don_hang,lv_don_hang);
end;

procedure Tf_quan_ly_don_hang.cbb_timChange(Sender: TObject);
begin
	da_tim:=false;
end;

procedure Tf_quan_ly_don_hang.cbb_timKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tf_quan_ly_don_hang.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
	luu_thong_so;
end;

procedure Tf_quan_ly_don_hang.FormCreate(Sender: TObject);
begin
	lb_lv(f_dl.lb_don_hang,lv_don_hang);
	doc_thong_so;
end;

procedure Tf_quan_ly_don_hang.lv_don_hangColumnClick(Sender: TObject;
	Column: TListColumn);
begin
	...
	begin
		...
		//f_dl.lb_don_hang_luu_lv(lv_don_hang,7);
end;

procedure Tf_quan_ly_don_hang.lv_don_hangCompare(Sender: TObject; Item1,
...
procedure Tf_quan_ly_don_hang.giai_ma_dl(vi_tri:integer);
var
	j: integer;
	vi_tri_cuoi: integer;
	ListItem: TListItem;
	...
	dong_dl:string;
begin
	dong_dl:=f_dl.lb_don_hang.items[vi_tri];
		for j := 0 to 9 do
		begin
		...
		ListItem := lv_don_hang_tim.Items.Add;
		with ListItem do
		begin
			Caption := a[0];
			SubItems.Add(a[1]);
