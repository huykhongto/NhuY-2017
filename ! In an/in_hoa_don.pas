...
uses Windows , {Messages,} SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
		{StdCtrls, Spin,} ExtCtrls, Clipbrd,LDNam_xu_ly_anh,LDNam_sys,ComCtrls,
		StdCtrls,System.Types,System.UITypes{, AppEvnts, xycommon};
...
procedure tao_ban_in(hop_ve:Tpaintbox; bcolor,fcolor:Tcolor; scale:single);
...
procedure initin_hoa_don;
function thong_tin_cong_ty(cv:TCanvas;rong:integer):integer;
function hoa_don(cv:TCanvas;rong,y:integer;so :string):integer;
function thong_tin_ngoi_sao(cv:TCanvas;rong,y:integer;st :TStringlist):integer;
function in_tong_cong(cv:TCanvas;rong,y:integer;st :TStringlist):integer;
function ten_cot_bang(cv:TCanvas;rong,y:integer):integer;
function in_danh_sach(cv:TCanvas;rong,y:integer;lv: TListView):integer;
function in_loi_cam_on(cv:TCanvas;rong,y:integer;mm :Tmemo):integer;
function in_ngay_ky_ten(cv:TCanvas;rong,y:integer;tg:string):integer;


//======================================== 

function bao_gia(cv:TCanvas;rong,y:integer;so :string):integer;
function in_tong_cong_bao_gia(cv:TCanvas;rong,y:integer;st :TStringlist):integer;
function in_ngay_ky_ten_bao_gia(cv:TCanvas;rong,y:integer;tg:string):integer;
function in_thong_tin_thanh_toan(cv:TCanvas;rong,y:integer;st :TStringlist):integer;

{the following procedures are needed for communication with in_hoa_don3D;
...
var monthnames : Tmonthnames =
  ('Jan','Feb','Maa','Apr','Mei','Jun','Jul','Aug','Sep','Okt','Nov','Dec');
	hoa_don_can:integer;
	xc:array[0..4] of integer;
	stt:byte;
	rong_net_ve:integer;
	can_giua_hoa_don:integer;
...
begin
  ...
  for i := 0 to maxgraf do with in_hoa_dondata[i] do
    begin
...
//======================================== 
function tinh_rong_dong(cv:TCanvas;dong:string): integer;
begin
	Result:=cv.TextExtent(dong).cx;
end;
//======================================== 
function tinh_cao_dong(cv:TCanvas;dong:string): integer;
begin
	Result:=cv.TextExtent(dong).cy;
end;
//======================================== 
procedure ve_doan_thang(cv:TCanvas;x0,y0,x1,y1:integer);
begin
	cv.MoveTo(x0,y0);
	cv.LineTo(x1,y1);
end;
//======================================== 
procedure ve_doan_ngang(cv:TCanvas;x0,y0,x1:integer);
begin
	cv.MoveTo(x0,y0);
	cv.LineTo(x1,y0);
end;
//======================================== 
procedure ve_doan_doc(cv:TCanvas;x0,y0,y1:integer);
begin
	cv.MoveTo(x0,y0);
	cv.LineTo(x0,y1);
end;
//======================================== 
procedure doi_rong_net(rong:integer;r:double);
begin
	rong_net_ve:=Round((r/rong_a4)*rong);
end;
//======================================== 
function in_ngay_ky_ten_bao_gia(cv:TCanvas;rong,y:integer;tg:string):integer;
var ty_le: double;
	...
	procedure in_can_giua_giua(xx:integer;s:string);
	begin
		cv.TextOut(Round(xx-tinh_rong_dong(cv,s)/2),y,s);
	end;
begin
 with cv do
 begin	
	Font.Name:='Tahoma';
	Font.Size:=kich_thuoc_font;
	Font.Style:= [];
	
	ty_le:=can_2_ben/rong_a4;
	x1:=Round(ty_le*rong);
	Font.Color:=clWebMediumBlue;
	in_can_giua_giua(rong-x1,tg);

	ty_le:=tinh_cao_dong(cv,chu_cao);
	cach:=Round(ty_le/10);
	cao:=Round(ty_le)+cach;
	y:=y+cao;
	Font.Color:=clWebLimeGreen;
	in_can_giua_giua(rong-x1,'Người Báo giá');
	Font.Color:=clWebDeepPink;
	in_can_giua_giua(x1,'Người đặt hàng');
	y:=y+cao*3;
	Font.Color:=clWebLimeGreen;
	in_can_giua_giua(rong-x1,'Lê Đăng Nam');
	Result:=y+cao*2;
	end;
end;
//======================================== 
...
begin
 ...
 begin	
	...
	y:=y+cao;
	Font.Color:=clWebLimeGreen;
	in_can_giua_giua(rong-x1,'Người xuất Hóa Đơn');
	Font.Color:=clWebDeepPink;
	in_can_giua_giua(x1,'Người nhận');
	y:=y+cao*3;
	Font.Color:=clWebLimeGreen;
	in_can_giua_giua(rong-x1,'Lê Đăng Nam');
	Result:=y+cao*2;
...
begin
 ...
 begin
	...
	Font.Size:=kich_thuoc_font;
	Font.Style:= [];
	Pen.Width:=rong_net_ve;
	Font.Color:=clWebLimeGreen;
	Pen.Color:=clWebMediumBlue;
	
	x1:=hoa_don_can;
	x2:=rong-hoa_don_can;
	ve_doan_ngang(cv,x1,y,x2);
	y1:=y;
	ty_le:=tinh_cao_dong(cv,chu_cao);
	cach:=Round(ty_le/10);
	cao:=Round(ty_le)+cach;
	y:=y+cach;
	x:=hoa_don_can+tinh_rong_dong(cv,' ');
	TextOut(x,y,mm.Lines[0]);
	y:=y+cao;
	TextOut(x,y,mm.Lines[1]);
	y:=y+cao;
	TextOut(x,y,mm.Lines[2]);
	y:=y+cao;
	TextOut(x,y,mm.Lines[3]);
	y:=y+cao;
	TextOut(x,y,mm.Lines[4]);
	y:=y+cao;
	y2:=y;
	ve_doan_ngang(cv,x1,y2,x2);
	ve_doan_doc(cv,x1,y1,y2);
	ve_doan_doc(cv,x2,y1,y2);
	Result:=y+cao;;
	end;
end;
//======================================== 
...
var ty_le: double;
  ...
  tong:int64;
	li: TListItem;
	ten_tk,dvt_tk,sl_tk,dg_tk:string;
	c:string;
	i:integer;
	procedure chuyen_doi(chuoi:string);
	begin
		c:=dinh_dang_so_c(chuoi);	
		x:=tinh_rong_dong(cv,c);
	end;
	procedure in_can_giua(r:integer;x:integer;s:string);
	begin
		cv.TextOut(x+Round((r/2)-tinh_rong_dong(cv,s)/2),y,s);
	end;
	procedure in_can_phai(s:string);
	begin
		cv.TextOut(rong-hoa_don_can-tinh_rong_dong(cv,s),y+tren,s);
	end;
begin
 with cv do
 begin
	...
	rong_min:=tinh_rong_dong(cv,' ');
	Pen.Width:=rong_net_ve;
	Pen.Color:=clWebGoldenRod;
	ty_le:=tinh_cao_dong(cv,chu_cao)/2; // Căn giưa cho doan thang va chu
	y1:=Round(ty_le)+y;
	y2:=y;
	y:=y1;
	TextOut(hoa_don_can+rong_min,y1,'STT');

	rong_dg:=Tinh_rong_dong(cv,'1.000.000.000')+rong_min*2;
	rong_sl:=Tinh_rong_dong(cv,'1.000.000')+rong_min*2;
	rong_dv:=Tinh_rong_dong(cv,'Thùng')+rong_min*2;
	rong_tt:=Tinh_rong_dong(cv,'10.000.000.000')+rong_min*2;
	rong_stt:=Tinh_rong_dong(cv,'STT')+rong_min*2;
	
	x1:=rong-hoa_don_can-rong_tt;
	xc[0]:=x1;
	in_can_giua(rong_tt,x1,'Thành tiền (Đ)');
	x1:=x1-rong_dg;
	xc[1]:=x1;
	in_can_giua(rong_dg,x1,'Đơn giá (Đ)');
	x1:=x1-rong_sl;
	xc[2]:=x1;
	in_can_giua(rong_sl,x1,'Số lượng');
	x1:=x1-rong_dv;
	xc[3]:=x1;
	xc[4]:=rong_stt+hoa_don_can;
	rong_t:=x1-xc[4];
	...
	y:=y2;
	in_can_giua(rong_dv,x1,'Đơn vị');
	y:=y+cao;
	in_can_giua(rong_dv,x1,'tính');
	y:=y1;
	x1:=x1-rong_t;
	in_can_giua(rong_t,x1,'Tên sản phẩm - dịch vụ');
	y:=y2;
	y:=y+cao;
	y:=y+cao;
	...
	begin
		...
		with li do
		begin
			in_can_giua(rong_stt,hoa_don_can,inttostr(stt));
			ten_tk:=SubItems.Strings[0];
			dvt_tk:=SubItems.Strings[1];
			sl_tk:=SubItems.Strings[2];
			dg_tk:=SubItems.Strings[3];
			tong:=Round(chuoi_so_t(sl_tk)*chuoi_so_n(dg_tk));
			tong_cong:=tong_cong+tong;
			TextOut(xc[4]+rong_min,y,ten_tk);
			in_can_giua(rong_dv,xc[3],dvt_tk);
			//TextOut(xc[3]+rong_min,y,dvt_tk);
			chuyen_doi(sl_tk);
			TextOut(xc[1]-x-rong_min,y,c);
			chuyen_doi(dg_tk);
			TextOut(xc[0]-x-rong_min,y,c);
			chuyen_doi(IntToStr(tong));
			TextOut(rong-hoa_don_can-x-rong_min,y,c);

			Pen.Width:=rong_net_ve;
			Pen.Color:=clRed;
			ve_doan_ngang(cv,hoa_don_can,y,rong-hoa_don_can);
			y:=y+cao;
			inc(stt);
		end;
	end;
	ve_doan_ngang(cv,hoa_don_can,y2,rong-hoa_don_can);
	ve_doan_ngang(cv,hoa_don_can,y,rong-hoa_don_can);

	ve_doan_doc(cv,rong-hoa_don_can,y,y2);
	ve_doan_doc(cv,hoa_don_can,y,y2);
	ve_doan_doc(cv,xc[0],y,y2);
	ve_doan_doc(cv,xc[1],y,y2);
	ve_doan_doc(cv,xc[2],y,y2);
	ve_doan_doc(cv,xc[3],y,y2);
	ve_doan_doc(cv,xc[4],y,y2);
	Result:=y+cao;
	end;
end;
//======================================== 
...
var ty_le: double;
	...
	function in_can_giua(r:integer;x:integer;s:string):Boolean;
	begin
		cv.TextOut(x+Round((r/2)-tinh_rong_dong(cv,s)/2),y,s);
	end;
begin
 ...
 begin
	...
	x1:=x1-rong_dv;
	xc[3]:=x1;
	xc[4]:=Tinh_rong_dong(cv,'STT')+rong_min*2+hoa_don_can;
	rong_t:=x1-xc[4];
...
var ty_le: double;
	...
	cao:integer;
	x:integer;
	x1,y1,x2,y2,x3:integer;
	
const
	ttns = 'THÔNG TIN CHUYỂN KHOẢN';

	function in_can_phai(xx:integer;s:string):Boolean;
	begin
		cv.TextOut(xx-tinh_rong_dong(cv,s),y,s);
	end;
begin
 with cv do
 begin
	...
	x1:=hoa_don_can;
	x2:=rong-hoa_don_can;
	ve_doan_ngang(cv,x1,y1,x2);
	
	Font.Color:=clWebSaddleBrown;
	TextOut(hoa_don_can*2,y,ttns);
	ty_le:=tinh_cao_dong(cv,chu_cao);
	cach:=Round(ty_le/10);
	cao:=Round(ty_le)+cach;
	y:=y+cao;
	...
	x3:=tinh_rong_dong(cv,'Thuế GTGT 10%: ');
	x3:=x3+hoa_don_can;
	x3:=x3+tinh_rong_dong(cv,'1.000.000.000.000');
	TextOut(x,y,'CÔNG TY TNHH THƯƠNG MẠI DỊCH VỤ LDNAM');
//	in_can_phai(x3,st.Strings[0]);
	y:=y+cao;
	TextOut(x,y,'NGÂN HÀNG:  EXIMBANK PHÒNG GIAO DỊCH GÒ VẤP, TP.HCM');
//	in_can_phai(x3,st.Strings[1]);
	y:=y+cao;
	TextOut(x,y,'SỐ TK:  220214851032954');
 //	in_can_phai(x3,st.Strings[2]);
	y:=y+cao;
 //	TextOut(x,y,'Bằng chữ: '+st.Strings[3]);
 //	y:=y+cao;
	y2:=y;
	ve_doan_ngang(cv,x1,y2,x2);
	ve_doan_doc(cv,x1,y1,y2);
...
begin
 ...
 begin
	...
	in_can_phai(x3,st.Strings[0]);
	y:=y+cao;
	TextOut(x,y,'Thuế GTGT 10%: ');
	in_can_phai(x3,st.Strings[1]);
	y:=y+cao;
	TextOut(x,y,'Tổng cộng: ');
	in_can_phai(x3,st.Strings[2]);
	y:=y+cao;
	TextOut(x,y,'Bằng chữ: '+st.Strings[3]);
	y:=y+cao;
...
begin
 ...
 begin
	...
	y:=y+cao;
	x:=hoa_don_can+tinh_rong_dong(cv,' ');
	TextOut(x,y,'Tên: '+st.Strings[0]);
	y:=y+cao;
	TextOut(x,y,'Địa chỉ: '+st.Strings[1]);
	y:=y+cao;
	TextOut(x,y,'Tel: '+st.Strings[2]);
	y:=y+cao;
	y2:=y;
	ve_doan_ngang(cv,x1,y2,x2);
	ve_doan_doc(cv,x1,y1,y2);
...
const
	bg = 'BẢNG BÁO GIÁ';
	function in_can_giua(s:string):Boolean;
	begin
		cv.TextOut(Round(can_giua_hoa_don-tinh_rong_dong(cv,s)/2),y,s);
		//y:=y+cao; 
	end;
	function in_can_phai(s:string):Boolean;
	begin
...
begin
 ...
 begin
	...
	ty_le:=can_le/rong_a4;
	hoa_don_can:=Round(ty_le*rong); // Tinh căn lề
	ty_le:=tinh_cao_dong(cv,chu_cao);
	cao:=Round(ty_le);
	in_can_giua(bg);
	...
	Font.Size:=kich_thuoc_font;
	ty_le:=tinh_cao_dong(cv,chu_cao);
	tren:=Round((cao-ty_le)/2); // can giua cho SỐ
	in_can_phai(so);
	cach:=Round(ty_le/10);
	y:=y+cao+cach;
	Result:=y;
...
var ty_le: double;
	...
	can_giua:double;
const
	ten_cong_ty = 'CÔNG TY LDNAM';
	//ten_cong_ty = 'LDNAM CO., LTD'; 
	dia_chi='TS: 217/2 Nguyễn Văn Nghi - P7 - Gò Vấp - TP.HCM';
	mst='MST: 0312805578                       ĐT: 01292726419';
	lhss='SẢN XUẤT - ĐIỆN TỬ'; // loại hinh san xuất
	slogan='LDNam phát triển bền vững cùng cộng đồng';
	phan_cach='--------------------------------------------------------------------';
	function in_can_giua(s:string):Boolean;
	begin
		cv.TextOut(Round(can_giua-tinh_rong_dong(cv,s)/2),y,s);
		y:=y+cao; 
	end;
begin
 with cv do
 begin
	logo:=TBitMap.Create;
	logo.LoadFromFile(duong_dan_goc+'/LDN.bmp');
	ty_le:=can_lo_go/rong_a4;
	y:=Round(ty_le*rong);
	
	ty_le:=cao_lo_go/rong_a4;
	cao:=Round(ty_le*rong);

	ty_le:=rong_lo_go/rong_a4;
	cach:=Round(ty_le*rong);
	ResizeBitmap(logo,cach,cao);
	Draw(y,y,logo);
	
	
 	doi_rong_net(rong,0.4);
	...
	Font.Color:=clWebMediumBlue;	
	in_can_giua(lhss);
	Font.Color:=clWebMagenta;
	in_can_giua(slogan);
	Font.Color:=clWebDarkOrange;
	in_can_giua(phan_cach);
	Result:=y;
	logo.free;
	end;
end;
...
begin
	if init then initin_hoa_don;
	if crosson then hop_ve_in.invalidate;
	hop_ve_in := hop_ve; cvmode := false;
	if doprint or dowmf then with hop_ve_in do with canvas do
     begin
       ofsz := font.size; ofst := font.style;
       font.size := 16; font.style := [fsbold];
       if doprint then s := 'Đang in ...' else s := 'Đang lưu ...';
  ...
  if doprint then xycanvas := printer.canvas else
                  xycanvas := hop_ve_in.canvas;
	cwidth := hop_ve_in.width*res; cheight := hop_ve_in.height*res;
...
begin
  ...
  in_hoa_dondata[igraph].ok := grafok;
  in_hoa_dondata[igraph].x1 := round((x1-xoff)/res);
  in_hoa_dondata[igraph].x2 := round((x2-xoff)/res);
  in_hoa_dondata[igraph].y1 := round((y1-yoff)/res);
  in_hoa_dondata[igraph].y2 := round((y2-yoff)/res);
  in_hoa_dondata[0] := in_hoa_dondata[igraph];
  {xyunzoom(igraph);}
  stil := st;
end;
...
begin {------------------------------------------------------------------------}
   ...
   with in_hoa_dondata[igraph].xaxis do
...
begin {++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++}
   ...
   dx := trunc(dl/7)+1; dow := dayofweek(xmi); {tel2 := 0;}
	 dt := encodedate(y,m,d) - (dow-mode) - 7.0*(dx-1);
   repeat
     xp := posx(dt); plotdate(0,0,0);
...
begin {------------------------------------------------------------------------}
   ...
   with in_hoa_dondata[igraph].yaxis[code] do
...
begin
  ...
  with in_hoa_dondata[igraph].yaxis[1] do
...
begin
  ...
  with in_hoa_dondata[igraph].xaxis do with graphs[igraph] do
...
procedure khoi_tao_may_in; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
var asp, 
	ty_le_trang : single;  // ty le cao/rong
	rong_trang,cao_trang
	,prw,prh,fx,fy,f : integer;
const fillfac = 1;//0.9;
begin
	 asp := cheight/cwidth; 
	 prfac := 0;
	 //---------------------------------------- dặt chiều cho ban in
	 if (asp>1) then printer.orientation := poportrait // Ban in nam doc
							else printer.orientation := polandscape; // ban in nam ngang
	 //---------------------------------------- 
	 rong_trang := printer.pagewidth; 
	 cao_trang := printer.pageheight; 
	 ty_le_trang := cao_trang/rong_trang;
	 //---------------------------------------- 
	 if (ty_le_trang>asp) then
		 begin prw := round(rong_trang * fillfac); prh := round(prw * asp); end
   else
		 begin prh := round(cao_trang * fillfac); prw := round(prh / asp); end;
   fx := prw div cwidth; fy := prh div cheight; f := min(fx,fy);
   if (f=0) then exit;
   prw := cwidth * f; prh := cheight * f;
   prxoff := (rong_trang-prw) div 2; pryoff := (cao_trang-prh) div 2;
   prfac := f;
end;
//======================================== 
...
begin
   ...
   if cvmode then exit;
	 khoi_tao_may_in; if (prfac=0) then begin beep; exit; end;

   hasbmp := xybuffers[0].bmp;
	 with printer do 
		ffac := sqrt(pagewidth*pageheight)/1000;
   ...
   try
		 ...
		 restorefont; 
		 //xypaintbox.repaint;
		 xypaintbox.Canvas.Draw(0, 0, b_ha); 
		 xycanvas.Draw(0, 0, b_ha);
			b_ha.Free;
			xypaintbox.repaint;
		 printer.enddoc;
...
procedure initin_hoa_don; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
...
begin                    { <--- }
  initin_hoa_don;           { <--- }
  {for C++ users: discard these two lines and call initin_hoa_don from your
