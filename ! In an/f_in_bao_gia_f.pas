...
uses
	...
	,LDNam_xu_ly_du_lieu,f_dl_f, Vcl.Menus,LDNam_xu_ly_anh,in_hoa_don,
	System.UITypes,LDNam_dl;
...
{$R *.DFM}
uses f_quan_ly_ngoi_sao_f, f_xem_truoc_in_f,f_quan_ly_thau_kinh_f;
...
begin
	...
	begin
		...
		begin
			for k:=1 to dl_ngoi_sao.sl_dl.Count-1 do  // tìm kiếm toàn bộ từ
			begin
				tu_tra:=bo_dau(tu_tra);
				tu_tra:= LowerCase(tu_tra);
				nghia_tu:=dl_ngoi_sao.sl_dl[k];
...
begin
	...
	begin
		...
		begin
			tim_kiem_sl(cbb_ten_thau_kinh.Text,dl_thau_kinh.sl_dl,sl);
